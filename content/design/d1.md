...
The font size is 11pt. It is easily readable but not too big. It follows [Butterick's advice](https://practicaltypography.com/presentations.html) to choose a font size so as to be able to fit about 12–15 lines of text on one slide.
...
The line spacing is generous: around 150% of the point size. This adds white space to the presentation, which helps with reading, and it limits the amount of stuff that can be written on one slide. It is fine to have 8–10 bullet points on a slide as long there are only a few words for each bullet point. 
...
[As Butterick says](https://practicaltypography.com/presentations.html), color should be used with restraint. A lot of colors, especially bright ones, are distracting. To reduce distraction, the template only uses grayscale. The text is in dark gray (85% black), not completely black, to avoid an uncomfortable degree of contrast. The bullet points are in lighter gray, to blend in the background. (The typical, bright Beamer bullet points should be avoided as they are distracting.) Colors are reserved for graphs and text alerts. 
...
\begin{frame}
\frametitle{Figure caption}
\includegraphics<1>[scale=0.4,page=1]{figures.pdf}%
\includegraphics<2>[scale=0.4,page=2]{figures.pdf}%
\includegraphics<3>[scale=0.4,page=3]{figures.pdf}%
\includegraphics<4>[scale=0.4,page=4]{figures.pdf}%
\end{frame}
````
...
The template comes with standard navigation buttons. The buttons have white background, just like the slides, and the button text is gray and in small font (9pt). The buttons blend in the slide, unlike the typical, bright Beamer buttons points that stand out and distract from the rest of the content.
